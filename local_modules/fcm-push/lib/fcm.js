...
FCM.prototype.send = function(payload, CB) {
    ...
    var eventId = self.eventIdCounter;

    if (CB) this.once(getEventKey(eventId), CB);
    ...
    operation.attempt(function(currentAttempt) {
        ...
        var request = https.request(self.fcmOptions, function(res) {
            ...
            if (res.statusCode == 503) {
                ...
                if (!operation.retry('TemporaryUnavailable')) {
                    self.emit(getEventKey(eventId), operation.mainError(), null);
                }
            ...
            function respond() {
                ...
                self.emit(getEventKey(eventId), error, id);
            }
        ...
        request.on('error', function(error) {
            self.emit(getEventKey(eventId), error, null);
        });
